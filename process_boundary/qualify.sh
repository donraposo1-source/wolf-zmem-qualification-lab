...
start_curator(){ runuser -u zcurator -- python3 "$TRUST/privileged_curator.py" "$PRIV" "$PROP" "$DB" & CURATOR=$!; for i in {1..50}; do [[ -S "$PRIV" && -S "$PROP" ]] && return 0; sleep .1; done; return 1; }
start_curator
...
import sqlite3,sys
c=sqlite3.connect(sys.argv[1]); a=c.execute("select state,promotions from memories where id='m1'").fetchone(); b=c.execute("select state,promotions from memories where id='m2'").fetchone(); print(a[0],a[1],b[0],b[1])
PY
)
[[ "$STATE" == PROMOTED && "$PROMOS" == 1 ]] && P09=PASS || P09=FAIL
[[ "$STATE2" == QUARANTINED && "$PROMOS2" == 0 ]] || exit 3
rm -f "$PRIV" "$PROP"; start_curator
