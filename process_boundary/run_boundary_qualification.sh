...
trap cleanup EXIT
trap 'rc=$?; printf "BOUNDARY_HARNESS_ERROR rc=%s line=%s command=%q\n" "$rc" "$LINENO" "$BASH_COMMAND" >&2' ERR
stage() { printf 'BOUNDARY_STAGE=%s\n' "$1"; }
...
root=pathlib.Path(sys.argv[1]); curator_uid=int(sys.argv[2]); worker_uid=int(sys.argv[3]); hash_before=sys.argv[4]; hash_after=sys.argv[5]
def load(name): return json.loads((root/name).read_text(encoding='utf-8'))
initial=load('worker-initial.json'); p07=load('p07-promote.json'); p07s=load('p07-status-before-replay.json')
p08=load('p08-deny.json'); p08s=load('p08-status.json'); p09a=load('p09-admin-replay.json')
p09w=load('p09-worker-replay.json'); p09s=load('p09-status-after-replay.json')
p12b=load('p12-before-kill.json'); p12a=load('p12-after-restart.json'); p07r=load('p07-after-restart.json')
results={
 'P01':bool(initial['P01']), 'P02':bool(initial['P02']), 'P03':bool(initial['P03']), 'P04':bool(initial['P04']),
 'P05':bool(initial['P05']), 'P06':bool(initial['P06']),
 ...
 'P10':bool(initial['P10'] and hash_before==hash_after), 'P11':bool(initial['P11']),
...
summary={**results,
 'WORKER_CAN_IMPORT_PRIVILEGED_SURFACE':not results['P01'], 'WORKER_CAN_WRITE_TRUSTED_DB':not results['P11'],
 'WORKER_HAS_OPERATOR_CAPABILITY':not results['P03'], 'WORKER_CAN_SELF_PROMOTE':not results['P05'],
 'CURATOR_CAN_PROMOTE':results['P07'], 'CURATOR_RESTART_SAFE':results['P12'],
 ...
 'EUR_SPEND':'0.00', 'curator_uid':curator_uid, 'worker_uid':worker_uid,
 'trusted_module_sha256_before':hash_before, 'trusted_module_sha256_after':hash_after}
